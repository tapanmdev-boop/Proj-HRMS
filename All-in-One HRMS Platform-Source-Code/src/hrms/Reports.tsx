...
export default function Reports() {
  ...
  // Real figures from the shared mock dataset — the same numbers rendered
  // below in the stat cards, so exporting and viewing never disagree.
  const employeeStats = useMemo(() => {
    const employees = mockData.employees;
    const active = employees.filter(e => e.status === 'Active').length;
    const avgTenureYears = employees.length
      ? employees.reduce((sum, e) => sum + (Date.now() - new Date(e.joinDate).getTime()), 0) /
        employees.length / (365.25 * 24 * 60 * 60 * 1000)
      : 0;
    return { total: employees.length, active, avgTenureYears: Math.round(avgTenureYears * 10) / 10 };
  }, []);

  const exportRows = useMemo(() => {
    const rows = mockData.employees.map(e => ({ Name: e.name, Department: e.department, Status: e.status, Position: e.position }));
    return department === 'All Departments' ? rows : rows.filter(row => row.Department === department);
  }, [department]);

  // The export reflects whichever report/filters are actually selected
  // (report name in the filename, department applied as a real filter) and
  // the same roster the stat cards above are computed from.
  const handleExportReport = () => {
    exportToCSV(exportRows, `${selectedReport}-${dateRange.replace(/\s+/g, '-').toLowerCase()}.csv`);
  };

  // No PDF library — this prints just the report content (chrome hidden via
  // the @media print rules in index.css) so the browser's own "Save as PDF"
  // produces the export.
  ...
  return (
    <div>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-5 gap-6">
        {/* Report Categories */}
        <div className="no-print md:col-span-1 bg-white rounded-xl border border-ivory-300 shadow-premium-sm p-4">
          <h3 className="text-[15px] font-semibold tracking-[-0.01em] text-ink-900 mb-4">Report Categories</h3>
        ...
        <div className="md:col-span-4">
          {/* Report Selection */}
          <div className="no-print bg-white rounded-xl border border-ivory-300 shadow-premium-sm p-4 mb-6">
            <h3 className="text-[15px] font-semibold tracking-[-0.01em] text-ink-900 mb-4">Available {currentCategory?.name}</h3>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-info-50 p-4 rounded-lg border border-info-100">
                  <div className="text-sm text-info-700">Total Employees</div>
                  <div className="text-2xl font-bold tabular">{employeeStats.total}</div>
                  <div className="text-xs text-info-500 mt-1">from the current roster</div>
                </div>
                <div className="bg-success-50 p-4 rounded-lg border border-success-100">
                  <div className="text-sm text-success-700">Active Employees</div>
                  <div className="text-2xl font-bold tabular">{employeeStats.active}</div>
                  <div className="text-xs text-success-500 mt-1">{employeeStats.total ? Math.round((employeeStats.active / employeeStats.total) * 100) : 0}% of roster</div>
                </div>
                <div className="bg-warning-50 p-4 rounded-lg border border-warning-100">
                  <div className="text-sm text-warning-700">Average Tenure</div>
                  <div className="text-2xl font-bold tabular">{employeeStats.avgTenureYears} years</div>
                  <div className="text-xs text-warning-500 mt-1">across the current roster</div>
                </div>
              </div>
      ...
      {showScheduleModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-ink-950/50 backdrop-blur-[2px] z-50">
          <div className="bg-white rounded-xl border border-ivory-300 shadow-premium-lg p-6 w-full max-w-md">
            <h3 className="text-[15px] font-semibold tracking-[-0.01em] text-ink-900 mb-4">Schedule Report</h3>
