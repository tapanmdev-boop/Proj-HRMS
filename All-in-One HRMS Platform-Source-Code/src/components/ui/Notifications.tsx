...
export const Alert: React.FC<AlertProps> = ({
  ...
  return (
    <div className={`border-l-[3px] rounded-lg p-4 ring-1 ring-inset ring-ivory-300 shadow-premium-sm ${variantClasses[variant].bg}`}>
      <div className="flex items-start">
        <div className="flex-shrink-0 [&_svg]:h-[18px] [&_svg]:w-[18px]">{variantClasses[variant].icon}</div>
        <div className="ml-3">
          {title && <h3 className="text-[13.5px] font-semibold text-ink-900">{title}</h3>}
          <div className={`text-[13.5px] text-gray-600 ${title ? 'mt-0.5' : ''}`}>{message}</div>
        </div>
        {onClose && (
...
export const Toast: React.FC<ToastProps> = ({
  ...
  // Toasts are always ink; the variant is carried by a small status dot so a
  // stack of notifications never turns into a row of loud coloured blocks.
  ...
  return (
    <div className="mb-3 max-w-sm rounded-lg bg-ink-900 px-4 py-3 text-[13.5px] text-ivory-50 shadow-premium-lg ring-1 ring-ink-700">
      <div className="flex items-center justify-between">
        <span className={`mr-3 h-1.5 w-1.5 shrink-0 rounded-full ${dotClasses[variant]}`} aria-hidden="true" />
        <div className="flex-1 mr-3">{message}</div>
        <button
          onClick={() => onClose(id)}
          className="text-ivory-50/50 hover:text-ivory-50 focus:outline-none"
          aria-label="Dismiss"
        >
          <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
            <path
...
export const Badge: React.FC<{
  ...
  return (
    <span
      className={`inline-flex items-center whitespace-nowrap font-medium tracking-[0.01em] ${
        variantClasses[variant]
      } ${sizeClasses[size]} ${rounded ? 'rounded-full' : 'rounded-md'}`}
    >
      {children}
    </span>
  );
